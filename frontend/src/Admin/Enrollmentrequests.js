...
export default function AdminEnrollmentRequests() {
  ...
  const handleCourseStatus = async (id, newStatus) => {
    try {
      await API.put(`/admin/course/verify-payment/${id}`, {
        status: newStatus,
      });
  ...
  const handleQuizStatus = async (paymentId, newStatus) => {
    try {
      await API.put("/admin/quiz/verify-payment", {
        paymentId,
        status: newStatus,
      });
      setQuizEnrollments((prev) =>
        prev.map((item) =>
          item.payment_id === paymentId ? { ...item, status: newStatus } : item,
        ),
      );
      Swal.fire({
        icon: "success",
    ...
    <div className="admin-dashboard-wrapper" style={{ marginRight: "450px" }}>
      <main className="admin-main-content">
        ...
        <div style={{ marginBottom: "50px" }}>
          ...
          <div
            className="card-panel admin-table-container"
                  ...
                  <tr key={item.enrollment_id} className="table-row-hover">
                    ...
                    <td>
                      <select
                        ...
                        onChange={(e) =>
                          handleCourseStatus(item.enrollment_id, e.target.value)
                        }
                      >
                  ...
                  <tr key={item.payment_id} className="table-row-hover">
                    ...
                    <td>
                      <select
                        ...
                        onChange={(e) =>
                          handleQuizStatus(item.payment_id, e.target.value)
                        }
                      >
                        <option value="Pending">Pending</option>
                        <option value="Approved">Approved</option>
